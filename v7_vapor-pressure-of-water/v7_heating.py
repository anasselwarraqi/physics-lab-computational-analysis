...
Date: February 2026
Purpose: Determine Enthalpy of Vaporization (Delta H) - Heating Phase.
"""
...
FILENAME = 'data/v7_heating.csv'  # Adjusted standard path
OUTPUT_DIR = 'plots'
PLOT_FILE = 'v7_vapor_pressure_heating.pdf'
TXT_FILE = 'results_vapor_pressure_heating.txt'
...
if df is not None:
    ...
    def resistance_to_temperature(R_ohm):
        # Pt100 formula: R(T) = R0 * (1 + A*T + B*T^2)
        A = 3.9083e-3
        B = -5.775e-7
        R0 = 1000.0
        # Inverse quadratic solution
    ...
    # Temperature
    sigma_T_sys = 0.3 + 0.005 * T_celsius
    sigma_R = 1.0
    sigma_T_R = sigma_R / 3.85
    sigma_T = np.sqrt(sigma_T_R**2 + sigma_T_sys**2)
    ...
    # Pass 1: Simple fit using only y-errors to get initial slope estimate
    popt_init, _ = curve_fit(linear_model, x_val, y_val, sigma=y_err, absolute_sigma=True)
    m_init = popt_init[0]

    # Pass 2: Calculate Effective Variance
    ...
    Delta_H = -slope * R_GAS  # Result is a ufloat (value +/- error)
    ...
    results_text = f"""
------------------------------------------------------------
Analysis Results: Vapor Pressure (Heating)
...
"""
    ...
    ax.errorbar(x_val, y_val, xerr=x_err, yerr=y_err, fmt='o',
                markersize=4, color='orange', ecolor='gray',
    ...
    ax.plot(x_fit, y_fit, color='darkgreen', linewidth=2,
            label=fr'Fit: $\Delta H = {Delta_H_kJ.n:.2f} \pm {Delta_H_kJ.s:.2f}$ kJ/mol')

    # Styling
    ax.set_title("Arrheniusplot Erwärmung: $\\ln(p)$ vs $1/T$", fontsize=14, fontweight='bold')
