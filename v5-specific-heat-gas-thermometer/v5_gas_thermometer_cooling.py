...
Date: February 2026
Purpose: The experiment uses a gas thermometer to determine the absolute zero temperature (Cooling Phase).
"""
...
FILENAME = 'data/v5_cooling.csv'
OUTPUT_DIR = 'plots'
PLOT_FILE = 'v5_gas_thermometer_cooling.pdf'
TXT_FILE = 'results_gas_thermometer_cooling.txt'
...
if df is not None:
    ...
    results_text = f"""
------------------------------------------------------------
Analysis Results: Gas Thermometer (Cooling)
...
"""
    ...
    ax.errorbar(T_celsius, p_hPa, yerr=SIGMA_P, xerr=SIGMA_T,
                fmt='o', color='blue', ecolor='gray', capsize=3, label="Messdaten")

    # Plot Fit Line
    t_line = np.linspace(min(T_celsius), max(T_celsius), 100)
    p_line = linear_model(t_line, slope.n, intercept.n)

    ax.plot(t_line, p_line, color="cyan",
            label=f"Fit: $T_0 = {T0.n:.1f} \\pm {T0.s:.1f} ^\\circ C$")

    # Labels and Styling
    ax.set_title("Gasthermometer Abkühlung: Druck gegen Temperatur", fontsize=14, fontweight="bold")
